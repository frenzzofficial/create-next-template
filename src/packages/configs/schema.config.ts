...
// Moved here from forms.config.ts to break a circular import:
// schema.config.ts -> forms.config.ts -> user.env.ts -> schema.config.ts
// (user.env.ts imports fullnameRules/emailRules/passwordRules from this
// file). Next.js's bundler tolerated the cycle; a stricter ESM evaluator
// (Vitest) does not - fullnameRules etc. were still undefined at the
// point user.env.ts tried to call .default() on them.
...
export const emailRules = z
  ...
  .refine(
    (email) => {
      ...
      return ALLOWED_MAIL_DOMAINS.includes(domain);
    },
